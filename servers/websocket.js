var primus = require('../node_modules/actionhero/node_modules/primus');
var UglifyJS = require('../node_modules/actionhero/node_modules/uglify-js');
var fs = require('fs');
...
var websocket = function(api, options, next) {
  ...
  server._start = function(next) {
    ...
    server.server =
      new primus(webserver.server, api.config.servers.websocket.server);
    ...
    api.log('webSockets bound to ' + webserver.options.bindIP + ':' +
    webserver.options.port, 'notice');
    server.active = true;
  ...
  server.sendMessage = function(connection, message, messageCount) {
    ...
    if (message.context === 'response' && message.messageCount == null) {
  ...
  server.compileActionheroClientJS = function() {
    var ahClientSource = fs.readFileSync(__dirname +
    '/../node_modules/actionhero/client/actionheroClient.js').toString();
    ahClientSource = ahClientSource.replace('%%DEFAULTS%%',
      'return ' + util.inspect(api.config.servers.websocket.client));
  ...
  server.writeClientJS = function() {
    if (api.config.servers.websocket.clientJsPath != null &&
      api.config.servers.websocket.clientJsName != null) {
  ...
  var handleDisconnection = function(rawConnection) {
    for (var i in server.connections()) {
      if (server.connections()[i] != null &&
        rawConnection.id === server.connections()[i].rawConnection.id) {
